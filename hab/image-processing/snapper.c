/**
 * This is a modified version of an internal Pi In The Sky file.
 * It is deprecated becasue the equivalent functionality has been implemented in
 * the current version of PITS.
 */
...

				{
					...
					if (access(filename, F_OK ) == -1)
					{
						...
						if ((fp = fopen(filename, "wt")) != NULL)
						{
							if (Channel == 4)
							{
								altval = GPS->Altitude; //Extra line
								...
								{
									fprintf(fp, "raspistill -st -w %d -h %d -a 12 -a \"Alt: %d\" -t 3000 -ex auto -mm matrix %s -o %s/$2/$1.jpg\n", width, height, altval, Config.CameraSettings, Config.Channels[Channel].SSDVFolder);
								}
							}
							else
							{
								...
								{
									fprintf(fp, "raspistill -st -w %d -h %d -a 12 -a \"Alt: %d\" -t 3000 -ex auto -mm matrix %s -o %s/$1.jpg\n", width, height, altval, Config.CameraSettings, Config.Channels[Channel].SSDVFolder);
								}
							}
